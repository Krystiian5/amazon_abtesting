...
def exploracion_df_abtesting(df, col_control):
    for categoria in df[col_control].unique():
        ...
        print(f"Los principales estadisticos de las columnas categoricas para el grupo {categoria.upper()} son:")
        display(df_filtrado.describe(include='O').T)
        print(f"Los principales estadisticos de las columnas numericas para el grupo {categoria.upper()} son:")
        display(df_filtrado.describe().T)
        print('------------------------------------')

# Para calcular normalidad
...
# Para calcular la homocedasticidad
...
def homocedasticidad (df,col_control, lista_metricas):
    for metrica in lista_metricas:
        ...
        if p_value > 0.05:
            ...
            print(f"Para la columna {metrica.upper()} las varianzas son homogéneas entre grupos, es decir, NO hay homocedasticidad")

# Calcular el test dde Mann Whitney
